...
**本地函数是一种嵌套在另一个成员中的私有方法，仅能从包含它的成员中调用它。**
...
本地函数和我们熟知的 Lambda 表达式 非常相似，比如上面示例中的本地函数，我们可以使用 Lambda 表达式实现如下：
...
本地函数支持范型，就像普通方法那样；而 Lambda 表达式不支持范型，因为它们必须被分配给一个有具体类型的委托变量（它们能够使用作用域内的外部范型变量，但那并不是一回事儿）。
...
![C# 中的本地函数-LMLPHP](https://c1.lmlphp.com/user/master/2020/11/24/b9d1c2c46c9a205f3e03bd24c3674a02.image)
...
![C# 中的本地函数-LMLPHP](https://c1.lmlphp.com/user/master/2020/11/24/2ca126ab56f20dd92e150bcb5cbcdc0c.image)
